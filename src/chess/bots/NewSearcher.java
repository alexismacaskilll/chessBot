...
public class NewSearcher<M extends Move<M>, B extends Board<M, B>> extends AbstractSearcher<M, B> {
	...
	public M getBestMove(B board, int myTime, int opTime) {
		int cutOff = cutoff;
		BestMove<M> bestMove = new BestMove<M>(Integer.MIN_VALUE + 1);
		M move = null;
		for (int i = cutOff; i <= ply; i++) {
			bestMove = jamboree(this.evaluator, board, i, Integer.MIN_VALUE + 1, Integer.MAX_VALUE, cutOff, move);
			move = bestMove.move;
		}
		return bestMove.move;
	}
	
	public static <M extends Move<M>, B extends Board<M, B>> BestMove<M> jamboree(Evaluator<B> evaluator, B board, int depth, int alpha, int beta, int cut, M move) {
		List<M> moves = board.generateMoves();
		if (move != null) {
			int position = moves.indexOf(move);
			moves.set(position, moves.get(0));
			moves.set(0, move);
		}
